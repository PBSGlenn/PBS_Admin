...
**Recent Changes** (2026-05-13 — v0.5.2):
...
**Last Updated**: 2026-05-13
**Version**: 0.5.2 (Adds `pdf_to_text` Tauri command via `pdf-extract` crate so `.pdf` clinical notes can drive AI report generation alongside `.md`/`.docx`. ReportSent panel gets a Source Files picker — explicit transcript + clinical-notes dropdowns with output-file filtering, override flag, and transcript persistence via `Event.transcriptFilePath`. Existing-report email button now resolves recipient at click time: previously-emailed → selected clinic → primaryCareVet fuzzy match → client.email. Phone-number fix in the vet-report cover letter template body (0402 857 064 → 0413 387 833) and a missing space in the HTML signature display text.)
