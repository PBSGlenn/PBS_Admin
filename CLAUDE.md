...
- **Call-site tagging** — every existing `EmailDraftDialog` usage forwards context to the log:
  - `ReportSentEventPanel` → `emailType="report"`, clientId + eventId
  - `ReportGeneratorDialog` → `emailType="report"`, clientId + eventId
  - `ClientHistoryDialog` → `emailType="history"`, clientId
  - `TasksOverview` (questionnaire reminders) → `emailType="questionnaire"`, clientId + eventId
  - `EmailInput` on Client view → `emailType="correspondence"`, clientId
- **EmailOptions extended** ([emailService.ts](src/lib/services/emailService.ts)): `clientId`, `eventId`, `emailType` are optional and forwarded through `EmailDraftDialogProps` → `sendEmail()` → `logSentEmailSafe()`. Helpers `sendConsultationReport()`, `sendVetReport()`, `sendQuestionnaireReminder()` accept an optional `context` arg and tag the right `emailType`. New `SentEmail`, `SentEmailInput`, `SentEmailType`, `SentEmailStatus` types in [types.ts](src/lib/types.ts).
- **Path B (delivered/bounced sync from Resend) intentionally parked** — `updateSentEmailStatus()` is the hook, waiting on a jurisdiction decision (in-app polling vs pbs-admin MCP vs Vercel webhook). Until B ships, every row stays `status="sent"`.

**Previous Changes** (2026-05-13 — v0.5.2):
- **PDF-backed clinical notes as a report source**: New `pdf_to_text` Tauri command ([lib.rs](src-tauri/src/lib.rs)) extracts text from `.pdf` files via the `pdf-extract` crate (added to [Cargo.toml](src-tauri/Cargo.toml)). The ReportSent panel's `generateReportMutation` now reads `.md` directly, converts `.docx` via pandoc, and extracts `.pdf` via the new command — so any of the three formats can drive AI report generation. Errors out cleanly on scanned/image-only PDFs ("PDF contained no extractable text").
- **Source Files picker on the ReportSent panel** ([ReportSentEventPanel.tsx](src/components/Event/ReportSentEventPanel.tsx)): two new dropdowns under SOURCE CONSULTATION let the user explicitly choose which transcript and clinical-notes files feed the AI. Auto-detection still pre-fills using the strict in-app filename convention (`{dateStr}_comprehensive-clinical*` / `practitioner-report*`), but the dropdowns surface every plausible candidate in the client folder (`.txt` for transcripts; `.md`/`.docx`/`.pdf` for clinical notes), filtering out output files (`*client-report*`, `*vet-report*`, `*consultation-report*`, `*prescription*`, `*invoice*`, `*referral*`, `*history_*`). A `clinicalNotesOverridden` flag prevents re-runs from clobbering an explicit user choice; resets when the consultation changes. Transcript selection persists to `Event.transcriptFilePath` so it sticks across sessions.
- **Vet email pre-fill on existing reports** ([ReportSentEventPanel.tsx](src/components/Event/ReportSentEventPanel.tsx)): Clicking the email button next to an existing report in the Existing Reports list now resolves the recipient at click time instead of inheriting whatever `recipientEmail` happens to be (the report-type-change effect clears it to `""` for vet reports with no clinic selected). Preference order: previously-emailed recipient from the report log → currently-selected vet clinic → `primaryCareVet` directory fuzzy match → `client.email` for client reports. Empty fallback still allows manual entry in the dialog's editable To field.
- **Email signature/template phone-number fix**: vet-report cover letter body in [emailTemplates.ts](src/lib/emailTemplates.ts) had a stale phone number (`0402 857 064`); replaced with `0413 387 833`. Same number in the HTML signature ([emailService.ts](src/lib/services/emailService.ts)) had the `tel:` href correct but the display text missing a space (`0413 387833`); now consistent.

**Previous Changes** (2026-05-12 — v0.5.1):
- **Ship B — Receiver-side reschedule reconciliation**: The Website Bookings card's Drift Report (shipped in v0.5.0-era as Ship A, read-only) now gains an **Apply Fixes (N)** action button. Clicking it updates `Event.date` to match Supabase's current `consultation_date + consultation_time` for every drifted row where the website's `rescheduled_count > 0`, and writes a structured audit-trail `Note` event per reconciliation (before/after/ref/count). The drift table gains a **Resched** column showing the count per row; rows with `rescheduled_count = 0` are surfaced for visibility but **skipped** by Apply Fixes — that's the defended path against stomping Captain's manual edits or import-time bugs (e.g. the Goss Event-251 +09 JST leak, since fixed at commit 2114175) that B has no positive signal to overwrite. A `force: true` flag is plumbed through the service layer for explicit operator override but not wired to UI.
- **Orphan-bookingRef fallback**: `importWebsiteBooking` now adopts pre-existing events with `NULL bookingReference` instead of creating duplicates. After `findEventByBookingReference` misses, the importer calls new helper [findOrphanEventByClientAndDate](src/lib/services/eventService.ts) — scoped to `Booking / TrainingSession / Consultation` event types on the matched client within ±1 day of the booking time — and writes the bookingRef back onto the closest match. Targets the Mushin PBS-G7QQ1VOS → Event 194 pattern identified in the 28 Apr audit (n=1 known production case).
- **Service-layer additions** ([bookingSyncService.ts](src/lib/services/bookingSyncService.ts)): new `reconcileBookingDate(row, options)` and `reconcileAllDrift(rows, options)` plus a `ReconciliationResult` shape. `BookingDriftRow` gains `rescheduledCount: number`, populated by `detectBookingDrift`. Direct `updateEvent` write path bypasses automation engine — no current rule fires on Note creation so the audit-Note's `event.created` trigger is a cheap no-op.
- **Zod schema bundled fix** ([schemas.ts](src/lib/schemas.ts)): `WebsiteBookingSchema` now carries `rescheduled_count`, `original_consultation_date`, `original_consultation_time` (the deferred one-liner from Ship A scoping in thread b8f13540). Same fields added to the runtime `WebsiteBooking` interface in [bookingSyncService.ts](src/lib/services/bookingSyncService.ts).
- **UX**: After Apply Fixes runs, the drift detector auto-reruns so the table shrinks visibly (or empties on full success). React Query invalidates `events` + `upcomingBookings` so Dashboard panels reflect the new dates immediately. Confirm dialog mirrors the existing Dismiss-confirmation pattern.
- **Scope held**: silent reconcile-on-Import deliberately NOT shipped — Ship B is click-driven for the first release. Promoting to silent is one wire-up in `syncAllWebsiteBookings` once a few real reschedules have landed cleanly. Per-row Apply also held; force-flag is the surgical-control escape via direct service call.

**Previous Changes** (2026-05-01 — v0.5.0):
- **Patient History Export**: New household-level patient-history export accessible via a **Generate History** button on the client view (next to the folder button). Opens [ClientHistoryDialog.tsx](src/components/Client/ClientHistoryDialog.tsx) — a modal with audience selector (Client / Vet / Other), date range, event-type chips, optional vet-directory pickup with addressee fields, optional cover note, and a chronological event-selection table with checkboxes. Filters and per-row toggles are independent: the type/date filters set default inclusion; manually toggling a row overrides that row only. **Reset to filters** clears overrides.
- **Per-Client (not per-Pet) scope**: Operates on the client/household because the schema has Client→Event but no Pet→Event link. Vet filtering is therefore manual — the user deselects events not relevant for the recipient. Authoritative household signalment is built from the Pet table via `buildSignalmentBlock()` and rendered as a `## Household` block at the top of the document.
- **DOCX → PDF → Email pipeline**: Generate writes `{surname}_{YYYYMMDD}_history_{audience}.docx` to the client folder via Pandoc + `General_PBS_Letterhead.docx` (auto-versions to `_v2`, `_v3` if a same-day file already exists). Action band pinned under the dialog header (always visible once generated) provides **Open DOCX**, **Convert to PDF** (Word COM, same naming pattern with `.pdf`), then **Open PDF** + **Email…** which opens the existing `EmailDraftDialog` with the PDF pre-attached and recipient pre-filled (vet-directory entry for vet audience, `client.email` for client).
- **HTML → Markdown for notes**: New [htmlToMarkdown.ts](src/lib/utils/htmlToMarkdown.ts) wraps `turndown` and strips internal HTML comments (e.g. `<!-- matchState: ... -->`, `<!-- emailLog: ... -->`) so machine-readable audit metadata never reaches an external recipient. Falls back to a tag-stripped plain-text version on parse error rather than crashing the export.
- **Service**: [clientHistoryService.ts](src/lib/services/clientHistoryService.ts) — `buildHistoryMarkdown()` (pure assembly), `generateHistoryDocx()` (Pandoc), `generateHistoryPdf()` (Word COM). Calls the underlying Tauri commands `run_pandoc_from_stdin` and `convert_docx_to_pdf` directly so it can use its own filename pattern without piggybacking on the consultation-report naming in `docxConversionService` / `pdfConversionService`.
- **Dependencies added**: `turndown` ^7.2.4, `@types/turndown` ^5.0.6.

**Previous Changes** (2026-04-24 — v0.4.3):
...
- [promptTemplates.ts](src/lib/prompts/promptTemplates.ts): `generateUserPrompt` emits an authoritative "Signalment (authoritative — source of truth)" block when `signalment` is present. A SIGNALMENT RULE appended to client-report prompts instructs the AI to copy the block verbatim and not extract signalment from the transcript or comprehensive report. Falls back to the old single-pet `Pet: (…)` line when `signalment` is omitted, so other callers are unaffected.
- [ReportSentEventPanel.tsx](src/components/Event/ReportSentEventPanel.tsx) and [ReportGeneratorDialog.tsx](src/components/Event/ReportGeneratorDialog.tsx): build signalment from the full pets array via `buildSignalmentBlock(pets)` and pass it to report generators. `ReportGeneratorDialog` now fetches pets via `getPetsByClientId`.
- **Scope**: practitioner/comprehensive clinical report intentionally unchanged — its cautious in-room wording is appropriate for the clinical record. Fix is isolated to client-facing output.

**Previous Changes** (2026-04-20 — v0.4.1):
- **Custom Medication Option**: [PrescriptionEventPanel.tsx](src/components/Event/PrescriptionEventPanel.tsx) medication dropdown now has a "✏ Custom medication..." entry at the top (separated from stock meds by `SelectSeparator`). When selected, shows an amber editable card with required Generic Name + optional Schedule Class (S2/S3/S4/S8). `buildCustomMedication(genericName, scheduleClass)` builds a synthetic `Medication` that flows through the existing DOCX pipeline — generic name goes straight into `{{medication_name}}`. Filename slug sanitizer (`\s+` → `_`, strip non-`[a-z0-9_-]`) keeps filenames clean for multi-word drug names.
- **Custom Dosage Directions (free text)**: New "Write custom dosage directions" checkbox above the dose grid. When ticked, Dose Rate + Frequency are replaced with a free-text textarea whose content is written verbatim to `{{dosage_directions}}`. Handles tapers, PRN, pulse therapy, and any multi-step schedule that can't be composed from a single dose rate. Structured fields remain the default for simple one-line prescriptions.
- **Formulation Bug Fix**: Dose concentration ("5mg per tablet") was collected but never passed to the DOCX template — prescriptions printed `Formulation: Tablet` instead of `Formulation: 5mg tablet`. Fixed by computing `formulationDisplay` in the mutation: strips trailing " per X" from the concentration string and appends the form type lowercase. `"5mg per tablet"` + `Tablet` → `"5mg tablet"`; `"2mg/ml"` + `Liquid/Solution` → `"2mg/ml liquid/solution"`. Both the DOCX template variable and the Event-notes summary now use the combined value.

**Previous Changes** (2026-04-18 — v0.4.0):
