...
**Recent Changes** (2026-05-12 — v0.5.1):
...
**Last Updated**: 2026-05-12
**Version**: 0.5.1 (Ship B — receiver-side reschedule reconciliation. Drift Report gains Apply Fixes (N) action; updates Event.date to match Supabase for `rescheduled_count > 0` rows and writes per-row audit Note. Rows with count=0 surfaced but skipped to protect manual edits. Orphan-bookingRef fallback in `importWebsiteBooking` adopts NULL-bookingRef events within ±1 day instead of duplicating. WebsiteBookingSchema gains `rescheduled_count` + `original_consultation_{date,time}` — the deferred Ship A bundle.)
